...
def select(pop, fitness):  # nature selection wrt pop's fitness
    idx = np.random.choice(np.arange(POP_SIZE), size=POP_SIZE, replace=True, p=(fitness) / (fitness.sum())) #物竞天择序号，进化
    pdb.set_trace()
    return pop[idx]
...
if __name__ == "__main__":
    ...
    pop = np.random.randint(2, size=(POP_SIZE, DNA_SIZE * 2))  # 随机初始化种群，matrix (POP_SIZE, DNA_SIZE)     DNA_SIZE * 2是X和Y两种编码
    # pdb.set_trace()
