def get_default_config():
    """The default configs."""
    # model_type = 'matching'  #指定模型类型
    model_type = 'embedding'  #指定模型类型
    ...
    graph_embedding_net_config = dict(
        ...
        use_reverse_direction=True, #如果是有向图则设成TRUE
        ...
        layer_norm=False,
        # set to `embedding` to use the graph embedding net.
        prop_type=model_type  #如果用嵌入网络就设置为 `embedding`
    )
    ...
    return dict(
        ...
        aggregator=dict(
            ...
            graph_transform_sizes=[graph_rep_dim], #128
            input_size=[node_state_dim], #32
            gated=True,
        ...
        data=dict(
            ...
            dataset_params=dict(
                ...
                n_nodes_range=[20, 20], # a tuple (n_min, n_max).  The minimum and maximum number of nodes in a graph to generate.
                p_edge_range=[0.2, 0.2], #The minimum and maximum edge probability.
                n_changes_positive=1, #the number of edge substitutions for a pair to be considered positive (similar).
                n_changes_negative=2, #当被替换的边达到两条是被认为两张图是不相似的
                validation_dataset_size=1000)), #验证数据集大小
        training=dict(
            batch_size=20,
            learning_rate=1e-4,
            mode='pair',
            ...
            # n_training_steps=500000, #50W  训练次数
            n_training_steps=1000, #1K
            # Print training information every this many training steps.
            print_after=100, #每隔多少训练步骤打印一次数据
