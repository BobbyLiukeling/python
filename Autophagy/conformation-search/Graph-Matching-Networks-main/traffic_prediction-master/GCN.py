...
class GCN(nn.Module):  # GCN模型，向空域的第一个图卷积
    ...
    def forward(self, data, device):
        ...
        graph_data = GCN.process_graph(graph_data)  # 变换邻接矩阵 \hat A = D_{-1/2}*A*D_{-1/2} 对数据进行归一化
        ...
        @staticmethod
        def process_graph(graph_data):  # 对矩阵归一化，这个就是在原始的邻接矩阵之上，再次变换，也就是\hat A = D_{-1/2}*A*D_{-1/2}
