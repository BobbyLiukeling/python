...
def main():
    ...
    # Loading Model
    my_net = GATNet(in_c=6 * 1, hid_c=6, out_c=1, n_heads=2) # 实例化模型

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")#指定训设备，CPU还是GPU

    my_net = my_net.to(device) #将模型送入设备

    criterion = nn.MSELoss() #定义损失函数

    optimizer = optim.Adam(params=my_net.parameters())#定义优化器
