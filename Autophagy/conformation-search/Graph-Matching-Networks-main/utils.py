...
def reshape_and_split_tensor(tensor, n_splits):
    ...
    """
    feature_dim = tensor.shape[-1] #节点特征维度
    tensor = torch.reshape(tensor, [-1, feature_dim * n_splits])# 2*256
    tensor_split = []
    for i in range(n_splits):
        tensor_split.append(tensor[:, feature_dim * i: feature_dim * (i + 1)])
    return tensor_split #将两两想比对的数据进行拆分
...
def build_model(config, node_feature_dim, edge_feature_dim):
    ...
    encoder = GraphEncoder(**config['encoder']) #将图数据进行编码
    aggregator = GraphAggregator(**config['aggregator'])  #图聚合
    if config['model_type'] == 'embedding': #两种比对方式
...
def build_datasets(config):
    ...
    if config['data']['problem'] == 'graph_edit_distance': #设置 数据编码的参数
        #加载、设置相关参数
        dataset_params = config['data']['dataset_params']
        validation_dataset_size = dataset_params['validation_dataset_size'] #数据集大小
