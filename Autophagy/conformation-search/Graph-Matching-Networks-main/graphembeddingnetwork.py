...
class GraphEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self._node_feature_dim = node_feature_dim #顶点向量的特征维度4
        self._edge_feature_dim = edge_feature_dim#边向量的特征维度8
        self._node_hidden_sizes = node_hidden_sizes if node_hidden_sizes else None #节点隐藏层宽度16
        self._edge_hidden_sizes = edge_hidden_sizes#边隐藏层宽度32
        self._build_model() #对节点和边进行全连接层编码

    def _build_model(self):#对节点和边进行编码
        layer = []
        layer.append(nn.Linear(self._node_feature_dim, self._node_hidden_sizes[0]))
        for i in range(1, len(self._node_hidden_sizes)): #这个是干什么的？？？
...
class GraphAggregator(nn.Module): #聚合层，顶点和边都聚合
    ...
    def build_model(self): #对节点和边进行聚合
...
class GraphEmbeddingNet(nn.Module):
    ...
    def __init__(self,
        ...
        self._layer_norm = layer_norm
        self._prop_layers = []
        self._prop_layers = nn.ModuleList() #传播层
        self._layer_class = layer_class
        self._prop_type = prop_type
        self.build_model()

    def _build_layer(self, layer_id): #创建第一个传播层
