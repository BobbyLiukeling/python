...
for query in args.query:
    ...
    simsearch_results=cosine_similarity(fps.iloc[query,:].values.reshape(1,-1),fps.dropna()) #得到相似性评分
    pdb.set_trace()
    ...
    # pdb.set_trace()
    if args.drop:
        simsearch_results.drop(args.query, axis=0).to_csv("query{}.csv".format(query))
    else:simsearch_results.drop(query, axis=0).to_csv("query{}.csv".format(query))