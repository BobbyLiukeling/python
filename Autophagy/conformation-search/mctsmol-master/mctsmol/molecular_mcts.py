...
class MolecularMCTS:
    ...
    def _get_rotatable_bonds(self, molecule):
        # pdb.set_trace()
        #O([H])[C@@](C([H])(C([H])(C([H])(C([H])(C([H])([H])[H])[H])[H])[H])[H])(/C(=C(\\[H])[C@@]1([C@]([C@@]2(C([H])([H])[C@]1(OO2)[H])[H])(C(/C(=C(/[H])C([H])(C([H])(C([H])(C(=O)O[H])[H])[H])[H])[H])([H])[H])[H])[H])[H])[H]  '[#7]->*'
        raw_rot_bonds = molecule.GetSubstructMatches(Chem.MolFromSmarts("[!#1]~[!$(*#*)&!D1]-!@[!$(*#*)&!D1]~[!#1]")) #化学键“#”对应的位置
    ...
    def search(self, state, num_simulations):
        ...
        # Perform simulations
        for i in range(num_simulations): #模拟的角度旋转数
            node = root_node
...
class _Node:
    ...
    def _get_child_states(self):
        ...
        if len(self.state[1]) < len(self._rotatable_bonds):
            ...
            theta = rdMolTransforms.GetDihedralDeg(conf, bond_to_rotate[0], bond_to_rotate[1], bond_to_rotate[2], bond_to_rotate[3])#返回原子 i、j、k、l 之间的二面角（以度为单位）
